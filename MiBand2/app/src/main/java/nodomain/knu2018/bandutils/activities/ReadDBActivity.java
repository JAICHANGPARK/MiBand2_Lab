...
//            mRecurrenceRule = recurrenceRule != null ? recurrenceRule : "n/a";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            patternGlobals.stream().forEach(a -> Log.e(TAG, "onCreate: " + a.getDate() + a.getKind()));
        }
...
//        initText();
    ...
    private void initSpinner(){
        //스피너 어댑터 설정
        ArrayAdapter adapter = ArrayAdapter.createFromResource(this, R.array.data_select_option,
                R.layout.spinner_item_test);
    ...
    public void onClickFloatingActionButton(View v) {
        //explosionField.explode(v);
        startActivity(new Intent(ReadDBActivity.this, AnalysisUserActivity.class));
    }
            ...
            request.enqueue(new Callback<ResponseAnalysis>() {
                ...
                public void onResponse(Call<ResponseAnalysis> call, Response<ResponseAnalysis> response) {
                    ...
                    if (response.isSuccessful()) {
                        Crashlytics.log(Log.INFO, TAG, "Read DB init Fetch.");
                ...
                public void onFailure(Call<ResponseAnalysis> call, Throwable t) {
                    Crashlytics.log(Log.INFO, TAG, "Read DB init onFailure.");
...
//    }
    ...
    private ArrayList<PatternGlobal> fetchGlobal() {
        ...
        sb.append(" SELECT '0' as kind, date, time, type, value FROM bloodsugar");
        sb.append(" UNION");
        sb.append(" select '2' as kind, date, time, (type1 || '-' || type2) as type, value from drug ");
        sb.append(" UNION");
        sb.append(" select '1' as kind, date, time, type, value from fitness ");
        sb.append(" UNION");
        sb.append(" select '3' as kind, date, time, type, exchange from meal ");
        sb.append(" UNION");
        sb.append(" select '4' as kind, date, time, type, duration from sleep ");
        sb.append(" ORDER BY ");
        sb.append(" DATE DESC");
        // 읽기 전용 DB 객체를 만든다.
     ...
     * @author : 박제창 (Dreamwalker)
     */
    private ArrayList<PatternGlobal> fetchASC() {
     ...
     * @author : 박제창 (Dreamwalker)
     */
    private ArrayList<PatternGlobal> fetchGlobalBloodSugar() {
        ArrayList<PatternGlobal> tmpArray = new ArrayList<>();
        StringBuilder stringBuilder = new StringBuilder();
        stringBuilder.append(" SELECT date, time, type, value FROM bloodsugar");
        ...
        while (cursor.moveToNext()) {
            ...
            tmpArray.add(new PatternGlobal("0", dateValue, timeValue, valueType, value));
        }
     ...
     * @author : 박제창 (Dreamwalker)
     */
    private ArrayList<PatternGlobal> fetchGlobalFitness() {
        ArrayList<PatternGlobal> tmpArray = new ArrayList<>();
        StringBuilder stringBuilder = new StringBuilder();
        stringBuilder.append(" select date, time, type, value from fitness ");
        cursor = db.rawQuery(stringBuilder.toString(), null);
        ...
        while (cursor.moveToNext()) {
            ...
            valueType = cursor.getString(2);
            value = cursor.getString(3);
            tmpArray.add(new PatternGlobal("1", dateValue, timeValue, valueType, value));
        }
     ...
     * @author : 박제창 (Dreamwalker)
     */
    private ArrayList<PatternGlobal> fetchGlobalDrug() {
        ArrayList<PatternGlobal> tmpArray = new ArrayList<>();
        StringBuilder stringBuilder = new StringBuilder();
        stringBuilder.append(" select date, time, (type1 || '-' || type2) as type, value from drug ");
        cursor = db.rawQuery(stringBuilder.toString(), null);
        ...
        while (cursor.moveToNext()) {
            ...
            valueType = cursor.getString(2);
            value = cursor.getString(3);
            tmpArray.add(new PatternGlobal("2", dateValue, timeValue, valueType, value));
        }
...
//        MenuItem spinnerMuneItem = menu.findItem(R.id.miSpinner);
//        final Spinner spinner = (Spinner) spinnerMuneItem.getActionView();
...
//                R.array.data_select_option, R.layout.spinner_item_test);
...
//            case R.id.miSpinner:
//
//                Log.e(TAG, "onItemClick: miSpinner " + "Clicked ");
//                Log.e(TAG, "onOptionsItemSelected: "  + item.getActionView().toString() );
